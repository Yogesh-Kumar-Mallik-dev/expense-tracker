...
export const GET = route(async (request: Request, context: Context) => {
  ...
  return ok(await services.transactionTags.list((await context.params).id, userId));
});
export const POST = route(async (request: Request, context: Context) => {
  ...
  return ok(await services.transactionTags.create(transactionId, input.tagId), 201);
});
export const DELETE = route(async (request: Request, context: Context) => {
  const userId = await requireUser(request);
  const input = schema.parse(await body(request));
  await services.transactionTags.delete((await context.params).id, input.tagId, userId);
  return empty();
});
