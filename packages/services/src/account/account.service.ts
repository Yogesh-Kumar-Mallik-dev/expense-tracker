...
import { createUuid, parseUuid } from "../shared/id";
import {
  accountIdSchema,
  createAccountSchema,
  updateAccountSchema,
...
export class AccountService {
  ...
  list(userId: string, includeArchived = false) {
    return this.accounts.listByUser(
      userIdSchema.parse(userId),
      includeArchived,
    );
  }
  ...
  async update(id: string, userId: string, input: UpdateAccountInput) {
    ...
    if (value.openingBalance !== undefined)
      updates.openingBalance = value.openingBalance;
    ...
    await this.accounts.update(accountId, ownerId, {
      ...updates,
