...
import { createUuid, systemClock } from "../shared";
import { createTransactionSchema, updateTransactionSchema } from "./transaction.schema";
import type { CreateTransactionInput, TransactionRecord, TransactionRepositoryPort, UpdateTransactionInput } from "./transaction.types";

export class TransactionService {
  constructor(private readonly repository: TransactionRepositoryPort, private readonly idFactory: IdFactory = createUuid, private readonly clock: Clock = systemClock) {}
  ...
  async create(input: CreateTransactionInput): Promise<TransactionRecord> {
    const value = createTransactionSchema.parse(input); const now = this.clock();
    const record: TransactionRecord = { id: this.idFactory(), ...value, createdAt: now, updatedAt: now, deletedAt: null };
    await this.repository.create(record); return record;
  }

  // Concurrency note: Safe read-only lookup; no returned value is persisted implicitly.
  get(id: string, userId: string) { return this.repository.findById(zUuid(id), zUuid(userId)); }
  ...
  list(userId: string, filters = {}) { return this.repository.listByUser(zUuid(userId), filters); }
  ...
  async update(id: string, userId: string, input: UpdateTransactionInput) {
    ...
    await this.repository.update(zUuid(id), zUuid(userId), { ...value, updatedAt: this.clock() });
  }
  ...
  async delete(id: string, userId: string) { await this.repository.delete(zUuid(id), zUuid(userId)); }
}

// Concurrency note: N/A - pure validation with no database access.
function zUuid(value: string) { return createTransactionSchema.shape.userId.parse(value); }
